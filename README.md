...
`src/styles/global.css`의 CSS 변수(`--color-*`)로 라이트/다크 팔레트를 관리한다. 포인트 컬러는 그린 계열이며, 다크모드는 사이드바의 토글 버튼으로 전환하고 `localStorage`에 저장된다. 인쇄(⌘/Ctrl+P) 시에는 사이드바가 숨겨지고 라이트 모드로 강제 전환되어 이력서를 PDF로 저장하기 좋게 최적화되어 있다.
